...
function Calendar({ userId }) {
  ...
  useEffect(() => {
    ...
    console.log("📤 [식사 기록 요청] userId:", userId, "date:", dateStr);

    axios.get(`http://localhost:8080/food-logs/${userId}?date=${dateStr}`)
      .then((res) => {
        console.log("✅ [식사 기록 응답]", res.data);
  ...
  useEffect(() => {
    ...
    console.log("📤 [칼로리 전체 요청] userId:", userId);
    axios.get(`http://localhost:8080/foods/total-calories?userId=${userId}`)
      .then((res) => {
        console.log("🔥 [전체 칼로리 응답]", res.data);
